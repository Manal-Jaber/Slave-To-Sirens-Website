...
const SliderTrack = () => {
    ...
    const [albumss, setAlbum] = useState('Terminal Leaches');
    useEffect(() => {
        
        axios.get('http://localhost:3001/admin/tracks').then((response) => {
           setData2(response.data.filter(e =>{
            // return e.album === document.querySelector("#album").value;

            return e.album === albumss;
        }));
        })
    },[data2, albumss,setData2]);
    ...
    //For Dynamic Website
    let sliderArr = data2.map(track => {
        return(<ReactPlayer className='react-player-disc' url={track.url}/>)
   });
    ...
    return (
        ...
        <div className="wrapper-disc">
           <div className="album-list-disc"> 
                ...
                {/*For Dynamic Website*/}
                <select className="album-disc" name="Album" id="album" value={albumss} onChange={(e) => {setAlbum(e.target.value)}}>
                {data1.map(albums =>{ 
                return (<option key={albums._id} value={albums.name}>
                    {albums.name}
                </option>)}
            )}</select>
            </div>
